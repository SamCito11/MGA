...
const ProgramacionClases = () => {
  ...
  const handleOpenCancelDialog = (horario, dia) => {
    setCancelData({ horario, dia });
    setCancelDialogOpen(true);
  };

  // Cerrar el diálogo de confirmación para cancelar
  ...
  // Confirmar la cancelación de la clase
  const handleConfirmCancel = () => {
    if (cancelData) {
      const { horario, dia } = cancelData;
      handleCancelClass(horario, dia);
    }
    setCancelDialogOpen(false);
    setCancelReason("");
    setCancelData(null);
  ...
  // Nueva función para cancelar una clase
  const handleCancelClass = (horario, dia) => {
    const diaKey = dia.toLowerCase().replace("é", "e").replace("á", "a");

    // Actualizar la programación eliminando la clase del día seleccionado
    setProgramacion((prev) =>
      prev.map((item) =>
        item.id === horario.id
          ? { ...item, [diaKey]: "" } // Eliminar la clase del día
          : item
      )
    );

    // Eliminar los estudiantes asociados a la clase cancelada
    const claseKey = `${dia}-${horario.hora}-${horario[diaKey]}`;
    setEstudiantesPorClase((prev) => {
      const updatedEstudiantes = { ...prev };
      delete updatedEstudiantes[claseKey];
      return updatedEstudiantes;
    });

    setSnackbar({
      open: true,
      message: `Clase de ${horario[diaKey]} cancelada para ${dia}. Motivo: ${cancelReason}`,
      severity: "success",
    });
  };
                        ...
                        >
                          ...
                          <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 1 }}>
                            <IconButton
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleOpenCancelDialog(horario, dia); // Abrir el diálogo de cancelación
                              }}
                              sx={{ p: 0.5 }}
                            >
                              <CancelIcon fontSize="small" /> {/* Usar el ícono de cancelar de GenericList */}
                            </IconButton>
                            <IconButton
                              size="small"
                              color="error"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteEvent(horario.id); // Restaurar funcionalidad de eliminar
                              }}
                              sx={{ p: 0.5 }}
                            >
      ...
      >
        ...
        <DialogActions sx={{ p: 0, justifyContent: "flex-end", gap: 1 }}>
          ...
          <Button
            ...
            disabled={!cancelReason.trim()} // Deshabilitar si no hay motivo
          >
            Confirmar
          </Button>
        </DialogActions>
